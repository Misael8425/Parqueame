...
import './SolicitudRechazadaDetail.css';
...
function SolicitudRechazadaDetail() {
  const { id } = useParams();
  ...
  const viewReason = () => {
    ...
    if (reason) {
      navigate(`/solicitud/${data.id}/razon-rechazo-detail`, { state: { razonRechazo: reason } });
    } else if (data?.id) {
      navigate(`/solicitud/${data.id}/razon-rechazo-detail`);
    } else {
      alert('No hay razón de rechazo disponible.');
    }
  };
    ...
    return (
      <div className="solicitud-detail-container">
        ...
        <main className="solicitud-detail-main-content">
          <p>Cargando detalles de la solicitud rechazada...</p>
        </main>
      </div>
    );
  }
    ...
    <div className="solicitud-detail-container">
      ...
      <main className="solicitud-detail-main-content">
        <h2 className="solicitud-detail-heading">
          ...
          <span className="status-rejected">{mapStatusToES(data.status)}</span>
        </h2>
        ...
        <div className="action-buttons">
          <button className="view-reason-button" onClick={viewReason}>Ver Razón de Rechazo</button>
...
export default SolicitudRechazadaDetail;